...
export const Detail = (props) => {
    ...
    return (
        <div className='select-none  mt-12 flex flex-col w-full  mx-auto justify-center items-center'>
            <div className='w-full px-[5%] lg:px-[15%] flex flex-row flex-wrap justify-start items-start font-bold font-Common text-[#29303e] text-[100%] lg:text-[1.5vmax]'>
            ...
            <div className=' mt-12 flex flex-col lg:flex-row w-full justify-center items-start'>
                <div className=' relative  flex flex-row justify-center items-center w-full'>
                    <MdNavigateBefore className='text-[#29303e] hover:text-[#34586e] text-[5vmin] m-2 cursor-pointer absolute left-0 top-[%45] z-50' onClick={() => { Prevclick() }} />
                    <Popup
                        modal
                        size={"large"}
                        trigger={<img className='shadow-md shadow-[#34586e] max-h-[90vh]' src={`${process.env.REACT_APP_BACKEND_URL}/${ROUTES.PRODUCTS}/GetImage?guid=${product ? product.uuid : ''}`} />} position="bottom">
                        {close => (
                            <div className="bg-transparent shadow-4xl   flex flex-col justify-center items-center rounded-lg ">
                                ...
                                <MdNavigateBefore className='text-[#29303e] hover:text-[#34586e] text-[5vmin] m-2 cursor-pointer absolute left-0 top-[%45]' onClick={() => { Prevclick() }} />
                                <MdNavigateNext className='text-[#29303e] hover:text-[#34586e] text-[5vmin] m-2 cursor-pointer absolute right-0 top-[%45]' onClick={() => { Nextclick() }} />
                            </div>
                        )}
                    </Popup>
                    <MdNavigateNext className='text-[#29303e] hover:text-[#34586e] text-[5vmin] m-2 cursor-pointer absolute right-0 top-[%45] z-50' onClick={() => { Nextclick() }} />
                ...
                <div className='w-full lg:w-1/2 px-8 mx-auto font-semibold text-[#29303e] font-Common flex flex-col justify-center   text-center items-center '>
                    ...
                    <div className='text-[#29303e] text-[150%] font-Common font-bold w-full flex justify-between items-center my-2 text-left'>
                        <h1> {product ? selected_record.name : ''}</h1>
                        <h1 className='mr-8'> {product ? product.price + " ₺" : ' ₺'}</h1>
                    </div>
                    <div className='text-[#29303e] text-[100%] font-Common font-normal  w-full flex whitespace-normal  justify-start flex-wrap items-center my-2 '>
                        <h1 className='text-left'> {product ? product.info : ''}</h1>
                    </div>
                    <div className='mt-4 flex  w-full flex-col lg:flex-row justify-between items-center font-bold text-[#29303e]'>
                    ...
                    <div className='mt-8  w-full flex justify-center items-center '>
                        {selected_record.products.length > 4 ?
                            <Slider ref={sliderRef}  {...settings} className="w-full flex items-center justify-center">
                                {selected_record.products.map((item, index) => {
                                    return <div className='w-full flex justify-center items-center' onMouseEnter={() => { setselectedProduct(index) }}>
                                        {index === selectedProduct
                                            ? <img key={item.uuid} className='mx-auto cursor-pointer shadow-lg shadow-[#34586e] w-auto h-[20vh]    transition-all ease-in-out duration-300' src={`${process.env.REACT_APP_BACKEND_URL}/${ROUTES.PRODUCTS}/GetImage?guid=${item.uuid}`} />
                                ...
                                {selected_record.products.map((item, index) => {
                                    return <div className='flex flex-row justify-center items-center flex-wrap  border-2 border-[#34586e] rounded-lg'>
...
function SampleNextArrow(props) {
    ...
    return (
        <div
            className='bg-white/40  p-4 rounded-3xl flex flex-row flex-nowrap justify-center items-center hover:text-[#34586e] text-[#29303e] transition-all ease-in-out duration-300 cursor-pointer'
